...
| 8 红线 A–I 全清（I 一字未动） | `scripts/ci-local.sh`（`CI-RC=0`，`[4/4] ci-local PASS`） | A/B/C/D/E/F/G/H/I 九条逐条 clean；`git diff scripts/ci-local.sh` 空＝**红线脚本本体一字未改**。测试通道两处改动（`ball_position.py` 定位法、`ui-smoke.sh` 扫视容量）之后各复跑一次红线，仍 `CI-RC=0`（`.smoke-tmp/redline-ballfix.log`、`.smoke-tmp/redline-uismoke-cap.log`） |
| 9 JVM 不回归只增不减 | 同一次 CI 的 `--rerun-tasks` 干净重跑，分模块 XML 计数 | **552 / 0 failures / 0 errors / 0 skipped**＝app 464 + byok 69 + contracts 19（515→552，+37）；新增三件激活套件 `ActivationCodeTest` 14、`ActivationGateTest` 10、`ActivationStoreTest` 9 |
...
见 `orders/METRICS.md` 末段与 `orders/ANYTOUCH-S5-delivery-ORDER.md` §4 v1.0.5 行：commit → annotated tag `v1.0.5` → push → Release 挂本字节 → 匿名 curl 回对 md5。
