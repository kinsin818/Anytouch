...
14. **E0 落的一条待裁（主窗发现，未擅自扩范围）**：裁决 S3-R4-1 原文只点名"开录/停止"，所以编译在跑时
    **「执行任务」与「步骤编辑」两个入口没有加互斥**。真实后果：编译归来会整本替换步序账，期间用户改的那一步
    被无声覆盖（不报错，但屏上"我改过的步骤没了"）。要不要把这两个入口并进同一条互斥（还是让编译那一跑持有到"用户下一次动作"为止），
    等一句话；主窗倾向=并进来（同一条判据已经在那儿，加两档成本极低），但不擅自扩裁决范围。见 `evidence/S3/slice-e0-compile-mutex.md` §7-2。
15. **E-key 真机轮挖出的一条跨层待裁（主窗发现，本批零改动两侧）**：编译器放行的动作词表 **宽于** 执行器支持集。
    ...
    **编得出、跑不动**。两侧支持集从来没有对拍过。本批不擅自改任一侧（收紧词表 vs 执行面补 `KEY` 是一句裁决的事），
    改为把判据钉进设备脚本：新增 **E5i**（产物步内出现执行面不支持的类型即当场红），首跑即抓到这条；
    真机 `[K4]` 那轮模型只交 click，故 E5i 绿——**间歇红，不是"偶发即不存在"**。全文见 `evidence/S3/slice-e2-k40-key.md` §5。
16. **31-A 落的两条（一条等放行、一条可覆）**：
    ① **31-B（A1/A2/A3，住 `executor/NodeTaskRunner.kt`）等老板一句话**。目标模式**不**覆盖这道门——
    `orders/ANYTOUCH-S31-ORDER.md` §3 字面写"待老板一句话"，老板 09-24 亦已明答"不算，另等一句话"，
    所以本窗在 31-A 验收全绿之后**不自动派** 31-B；设备争用已消解（真机当前不在位，模拟器可用），这一句随到随派。
