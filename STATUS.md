...
| S4 上线 | ✅ **S4 批定案（09-24 傍晚老板裁"接受现状"，原文逐字入 ORDER §2-4；定案前两态照录：S4-a 触顶停手照报 + S4-b DRAFT 落盘）**。S4-a：新脚本 `scripts/s4-fullflow-k40.sh`（S0~S8 十八断言含 S0b 设备独占锁——r4/r5"假终止通知→双驱动互污"血账换来的机器锁；S1c BACK 收键盘级联=MIUI 新设备事实 F-4/F-5）；K40 真请求 6/8（老板再改钉后）：TIMEOUT 拒×2、EMPTY_ACTIONS 拒×2、成×2（其一为 ctx off 受控诊断，`compile ok steps=1` 21 秒回）——同措辞方差在册（午后 ctx-on rows=29 四连拒 vs 正午 rows=30/31 成功反例，两读法未定，不据此改产品默认）。**r8 实跑（17:07~17:10，老板令"批级重钉 8、单命令跑全链"）**：S0~S2 六格同轮全绿（独占轮连续第二整段、预检修口全部生效），S3a 又 TIMEOUT（transport·第 2 次，rows=29 采集健康），脚本判据当场大声停；剩余额度 2<全链 3，按 §2-4 再停手照报，不放半链轮、不烧管道码冒充 RC（新账 F-8：`| tee` 时 wrapper 退 0≠脚本退 0）。通道面绿读数逐条在档，产品代码零改动。S4-b：`docs/onboarding-gumroad-en.md` DRAFT v1（标签映射逐字取代码、每句承诺对磁盘、飞行模式回放一句挂 pending 不冒领、非母语自标；背书十行已逐行独立对盘）。**终局：老板裁路②"接受现状"（路① +3→11 放 r9 作废未执行，额度账定格 6/8、剩 2 不花）**——超时/空数组归因上游端点时段性劣化，产品通道面全绿、失败必须说话行为全对；S4-b 两处 pending 已按现有背书转正（全文 pending/DRAFT 清零，边界不洗：行为证据≠抓包、真机整链绿轮没有），定案口径=裁决结案而非判据通过。详见 `evidence/S4/slice-s4a-fullflow.md` §1/§5/§7 + ORDER §2-4/§5 | 同左 | 军令 `orders/ANYTOUCH-S4-packaging-ORDER.md`、证据 `evidence/S4/slice-s4a-fullflow.md`（+ raw r1~r8、d1 九份带时戳）、话术 `docs/onboarding-gumroad-en.md` |
| S5 交付面三缺（上架前置门） | 🟡 **工单已开（老板 S5-R3，09-24 晚令"做完再上架"）；开工顺序=老板 S5-R5 令 c→a→b**。S5-c **✅ 本批落地**：`LICENSE`=gnu.org 官方 GPLv3 全文（35149B/674 行，尾部 why-not-lgpl 段核对一致）；`README.md` 全英文重写（旧 6 行中文留 git 历史），使用说明/开源声明/隐私政策三段齐 + Known limitations 逐条对盘（compileSdk 36/targetSdk 34/minSdk 26 取自 build 文件、"真机整链绿未达成"按 S4 定案口径直引证据原文、UI 英文仅陈述"本构建未随发"不做路线图承诺）；中文残留扫描=0、Key 字样扫描=0、与 S4-b 话术同源不矛盾。S5-a **靶机改判已落（老板 S5-R6）**：主窗摸底 K40 421 包、Gmail/相册/Discord 零命中呈报后，老板裁"别在国行K40上折腾了……直接在AVD的Pixel原生镜像（Android14/15那俩，自带GMS）上测模板"——验收面改为 **avd34+avd35 双 GMS 镜像**（实测 avd34 在架 `com.google.android.gm`/`com.google.android.apps.photos`/`com.android.vending`；`com.discord` 需装 APK=老板已令），K40 轮次与网络额度两账皆零；口径边界不洗：AVD≠海外真机，实证只声称 AVD 原生镜像口径（与 T3-R4 已裁验收面同向）；**S5-a 相册清理全链实证 ✅（老板 S5-R7 令"相册=全链实证 100%"，09-25 达成）**：`photos_cleanup` 模板在 avd34+avd35 **各 5 轮全链绿**（回执 `S1SMOKE ok=9 total=9 stopped=false` + 磁盘终判篓内外双真空 + T0a/b/c 装载面绿 + 脏 id fail-closed），逐字 raw 10 份入 `evidence/S5/raw/`；实证件 `evidence/S5/photos_cleanup-device-proven-avd.md` 落盘后，`PresetTemplateLibrary` 将 photos 翻 `DEVICE_PROVEN_ON_AVD`（反虚标锁 `PresetTemplatesTest` 正向分支首次走通，同步修其相对路径 `../evidence/S5`），模板 JVM 用例 8/8 绿；Gmail/Discord 仍 STRUCTURAL_ONLY（不随本件翻档、不代人登录）；踩实四雷写死测试脚本注释（种子须真实尺寸/两张须异像素否则内容级折叠/逐轮换名否则 MediaProvider 不重新建行/移篓=改名删行且双弹层壁每冷启须 dismiss）；产品代码仅 `PresetTemplateLibrary.kt` 一行档位+一处测试路径修正，资产/词表零坐标、执行零网络零额度；S5-b **✅ 本批落地（老板 S5-R8 三裁放行，09-25）**：remote=`https://github.com/kinsin818/Anytouch.git`（公开仓），main+tag v1.0.0 已推、远端与本地逐字同点（`64fc1ec`）；全史 103 commit Key 形态扫描=9 字面全为合成 fixture、与真 Key 池重叠 0（老板裁"视同零命中"）；Release=`github.com/kinsin818/Anytouch/releases/tag/v1.0.0`，资产 `app-debug.apk` 9,374,132B、md5 `054993cc700fa3f7d3344bc85638d206`（HEAD 现构建，非实证轮旧包），匿名下载回对 md5 一致；文案三要点齐（核心功能/已知限制/隐私），首发按 S5-R8 改判挂 **debug 签名测试包**——"无缝升级"经勘误改真话"签名版需先卸载重装"，keystore 保管案**待签名版回炉**。登录授权全程老板本人（GCM 弹窗+gh 设备码 7733-D6C0，主窗未触 Token 本体）。三缺至此全闭，R3"做完再上架"之门解开；**Gumroad 上架动作本身仍归老板面**，且账面新边界在册：公开的是 debug 测试包，正式签名版未发。缘起=DuMate 二审实测盘上 R3-1/R3-3/交付物2·3 零实现而初审未查（$6.99 付费点无货）。同批已关前置：二审门槛1 MediaProjection=**S5-R1 第一版直接豁免**（纯节点树+用户自带 VLM 兜底）；门槛2 真机口径=**S5-R2 确认 T3-R4 为最新、初审"生命线"段作废**（Samsung/Moto 缺口已点名写进 S4-b 话术风险段）；门槛4 映射表=**S5-R4 已执行**（`evidence/S5/nim-model-map-rewrite.md`，可用池经 S5-R5 收口=实测 200 的 10 款）；**S5-c(UI 英化) 批已落地（09-25 晨，老板军令原文"把所有UI文案全改成纯英文，去掉中英切换，改完重新打APK发v1.0.1 Release，全是英文界面，海外用户用着不违和"）**：全量上屏文案英文化（app 侧 25 源文件 + byok 侧 4 源文件，只改显示串与日志/KDoc 文本），**执行器·编译器·安全门禁·凭据存储逻辑零改动**（军令第③条）；盘上本无中英切换开关（勘误在册，"去掉切换"按意图执行=不虚构删除）；三模板按钮 label 改由 `PresetTemplateLibrary` 单一真源取字（此前 UI 硬编码与库脱节）；`versionCode 1→2`、`versionName 0.1.0-s1→1.0.1`。需求⑤两腿自证：静态腿=剥注释后 99 条 CJK 字面量逐条归六族豁免（KDoc／`S#SMOKE` 日志／高危词表／编译系统提示词／模型上行块／测试夹具，消费方证据到 file:line，`evidence/S5/cjk-literal-inventory.md`）；动态腿=新脚本 `scripts/ui-english-sweep.sh` 顶到底走位（顶/底锚点 + ≥2 不同状态判据，先抓到自家"半屏假绿"一次并自曝）扫得 55 条上屏文案、**CJK 命中 0**。英文构建字节上复跑 S5-a 全链判据链 **13/13 × 5 轮全绿**（`evidence/S5/english-build-device-revalidation.md`；同构建 `scripts/ui-smoke.sh` **41/0/0、0 SKIP**，本批改该脚本 3 处英文字面量断言后重跑；`byok-smoke` 7 行同族改动**未重跑**=欠格在册，跑它须清在机凭据或发真请求，折中以走位扫描抓到 `Key saved on this device: ***JgYp` 逐字为凭）；同批自曝两处**裁判**缺陷并只加不减地修好：高危确认"盲投"无撤窗复核、磁盘终判抢在 Photos 异步删除之前=假红）。JVM **431/0** 不回归（app343/byok69/contracts19，分模块 XML 计数）、ci-local 红线 **A–I 全清 PASS**、红线 C 被英文文案撞过一次（`[Uu]pload`）按"改选手不改裁判"改措辞而非松红线；测试与红线跑完 APK md5 仍 `86833471bd0c134491ee78991c224d3b`（9,348,476B）=发布字节与被扫字节同一份。v1.0.1 Release 已真建（09-25 收口，见 `orders/METRICS.md` 末段"发布收口"勘误：先写口径后建 Release，现已实证补齐）：commit `5b2c3c1` 推 `origin/main`、tag `v1.0.1` 推远端、`https://github.com/kinsin818/Anytouch/releases/tag/v1.0.1` 标 Latest，匿名下载回对 9,348,476B / md5 逐字一致；v1.0.0 不回改、资产原样在册。K40 零触碰、S4 额度 6/8 定格未动、真请求额度零消耗。**待裁一件**：高危确认面板正文仍是工程黑话（`High-risk action: DELETE:delete` / `matched field TEXT`），军令第①条"地道英文"严格说这条没达标，但它是安全闸唯一上屏归因，改写须重打+重扫+重跑五轮——本批按"已验证字节不动"交付 → **老板 S5-R10 已裁并已执行（见下 v1.0.2 段）**。**v1.0.2（09-25 上午，S5-R10 裁 1/裁 2 两小改落地并发布）**：①面板去黑话——显示串收进单一真源 `HighRiskPanelCopy`（七类各一句人话 + "不答就不跑"句），`OverlayUi` 不再把规则 id／命中字段名拼上屏，归因改由日志行与执行回执承担，**判定链零改动**（`HighRiskMatcher`+`HardcodedHighRiskRules` 原样），新增 JVM 文案锁 `HighRiskPanelCopyTest` 四条；②买家件剥离中文内证——`docs/onboarding-gumroad-en.md` 内部状态头 + 附录 B 中文背书表逐字搬入内部件 `evidence/S4/s4b-buyer-copy-backing-table.md`，`docs/usage-notes.md` 标"内部件·不随发布件分发"（路径不动以保 RULINGS T3-R2 锚可解析），两份买家件 CJK 行=0。`versionCode 3`/`versionName 1.0.2`，APK md5 `775ebb89bef9f6e8632c4b07d0ab2eb6`（9,290,901B）。验证面（全在 avd34，K40 零触碰）：红线 A–I 全清、**JVM 435/0/0/0**（app347/byok69/contracts19，分模块 XML）、`ui-english-sweep` 55 串 **CJK=0**、`ui-smoke` **41/0 且 SKIP=0**、Photos 全链 **13/13 × 5 轮**；裁 3 的 `byok-smoke` 7 行按令**欠格未跑**（并入下一次真机批）。**同批自曝并修一处同源测试通道缺陷（红记测试面不记产品）**：run1 轮 4 "焦点见窗即截屏"抓到无面板内容帧（面板底色像素 0 vs 正常轮 104548）→ 按屏定位量不到钮 → 退 v1.0.1 死坐标落空 → 产品按 fail-closed 15s 默认拒（`ok=8/9`+`SAFETY_GATE_BLOCKED`，产品行为正确）；修法=睡 1s→拍→量→最多三拍 + 兜底坐标更新为 v1.0.2 实测 `(729,1391)` + 检测器输出 `dark-panel-px` 计数，12 张历史面板截屏离线回归逐字对点，run2 五轮全绿零兜底零补点（`evidence/S5/v102-device-revalidation-and-panel-race.md`）。**发布账（先建后记）**：commit `bb34160` 推 `origin/main`、tag `v1.0.2` 推远端、`https://github.com/kinsin818/Anytouch/releases/tag/v1.0.2` 已为 `releases/latest`，资产匿名回对 9,290,901B / md5 逐字一致；v1.0.0/v1.0.1 未回改（原资产 9,374,132B / 9,348,476B 在册）。遗留不变：正式签名版与 keystore 保管案（上架前回炉）、Gumroad 上架动作归老板面、买家件"未经母语校对"仍待办、S4 额度 6/8 定格未动、本批真模型请求零消耗。 **v1.0.3（09-25 午后，S5-d 重复执行循环已发布·先建后记）**：界面两框 **Repetitions (1-100)** / **Interval seconds (1-60)** + 一勾 **Repeat without asking**（缺省 1 轮 / 5 秒 / 不勾）。轮次时序=上一轮整跑完 → 等 `interval ±1s` 抖动 → 下一轮；急停球全程可点、按下即停所有后续轮次；高危确认仍**每次命中都弹**（裁 C：勾了才允许本轮已放行过的同一类别在后续轮自动放行，且每次自动放行都写留痕；没放行过的新类别照旧弹窗）。**判定链零改动**（`NodeTaskRunner`／`HighRiskMatcher`／`HardcodedHighRiskRules`／编译链一行未改，`RepeatWiringLockTest` 四条机械锁钉死）。`versionCode 4`/`versionName 1.0.3`，APK md5 `93b284d48fa52c2daa01b5ff6517e985`（9,335,775B）。验证面（全在 avd34，K40 本批零指令）：JVM **452/0/0/0**（app364+byok69+contracts19，干净重跑分模块 XML）、`ci-local` RC=0 红线 A~I 全清、`s5d-repeat-loop-smoke.sh` run4 **15/15**、`ui-smoke.sh` run3 **51/0/0 且 SKIP=0**、`ui-english-sweep` RC=0、Photos 全链 **13/13 × 5 轮**、`device-smoke.sh` 15 格 **ALL PASS 两连绿**（同批自曝并修好三处测试通道账：C3 六条设备事实／C5 搜索索引失效换靶／C7 先假绿后两连假红→修后首点 5/5，账在 `evidence/S5/s5d-repeat-loop-device-run.md` §7）。**发布账**：commit `87e6e9d` 推 `origin/main`（`bc4dd3b..87e6e9d`）、annotated tag `v1.0.3` 推远端、Release `https://github.com/kinsin818/Anytouch/releases/tag/v1.0.3` 现为 latest（`published=2026-09-25T06:43:39Z`），资产经**匿名 curl（无凭据）** HTTP 200 / 9,335,775B / md5 与本地发布件逐字一致；v1.0.0~v1.0.2 未回改（资产 9,374,132 / 9,348,476 / 9,290,901 B 原样在册）。对外文案只写"每轮之间等待你设定的秒数，带 ±1 秒随机抖动"，"避免被平台检测"一句未进任何对外件。**一条产品观察如实上报未改**：急停球停靠位随窗态变化（实测三枚 frame，Δy 最大 442px）——军令"急停/门禁零改动"与 `OverlayUi.kt:60-61` 的三台设备实证锚同时生效，要动须另裁。下一批=S5-e 收尾三功能（v1.0.4，老板三裁已入 RULINGS S5-R12）：只准节点字段禁坐标／只重试非高危、高危每次命中照旧弹／先发 v1.0.3 再做 v1.0.4（已发）。 **v1.0.4（09-25 晚，S5-e 收尾三功能·老板三裁 S5-R12 全落·先建后记）**：三裁逐条落点——①错误重试**只碰非高危步骤**（`StepRetryPolicy` 判据层：同一步 locator-miss 最多补到 3 次尝试，高危步骤一律 `retry=off` 且每次命中照旧弹二次确认，超时默认拒这条一行未动）；②"我的任务"列表**走同一个现有落账口**（`AndroidSavedTaskDisk` 只读写既有的那份磁盘账，未起第二存储、未开第二派发通道，`S5ESMOKE saved task op=save|load|delete` 逐条留痕）；③手动补步骤第一版**只允许节点/输入/等待类**，面板明写"no x/y to fill in here"，**坐标点击入口在 UI 与写口两处都不存在**（判据 F14c 机器锁：树上永不出现 `step_insert_(x|y|coord|point)` 类节点）。判据 1–10 全对号：新脚本 `scripts/s5e-final-three-smoke.sh`（16 格 36 断言）run6 **36 PASS / 0 SKIP / 0 FAIL、SCRIPT_RC=0**；"保存→冷启→载入→回放"磁盘双缺席、载入不绕安全门禁（F15/F16 负例）、手填高危照样弹、插步回放同形、越界拒。`versionCode 5`/`versionName 1.0.4`，APK md5 `eb4a1504b5373357a0773f838fee3da6`（9,356,437B）＝**一枚字节贯穿全批**（构建 17:29 后 `find … -newer apk` 空集，跑前对账"机上包=当前源码"）。验证面（全在 avd34，K40/K80 零触碰）：JVM **515/0/0/0**（app427+byok69+contracts19，452→515 三切片 +11/+27/+25）、红线 A–I 全清 RC=0、旧账不回归同一枚字节复绿（device-smoke C 系列 ALL PASS、ui-smoke 50/0/1SKIP、s5d L1~L7 **15/15**、Photos 全链 **13/13 × 5 轮**、`ui-english-sweep` 三段时间新增"插步面板层"**85 串上屏文案 CJK=0**）。**本批五把假红全部先归因再改判据，红记测试通道不记产品**（`evidence/S5/s5e-final-three-device-run.md` §4），并挖出一条新的设备事实：**自家 Compose 面上 `adb shell input tap` 实际落点比 `uiautomator dump` 报的 y 低约 85px**，按报的坐标点中心**根本不触发 onClick**——此前所有"点得动"的绿从未证明过这条通道，修法=偏移梯（85/0/45/130）+ 命中判据换成产品自己的状态变化（它的 testTag 进树）；另 `install -r` 后进程不自动重启，**pidof 读空是"进程真被收走"的正证**（旧断言把正证当失败，run4 假红）。发布账（先建后记）：commit `e6c4ebe` 推 `origin/main`（`a1d7166..e6c4ebe`，同批带上此前未推的 `fb5fe20`/`2a739dc`/`2fa37c1` 三枚实现件）、annotated tag `v1.0.4` 推远端、Release `https://github.com/kinsin818/Anytouch/releases/tag/v1.0.4` 现为 latest（`published 2026-09-25T11:45:12Z`），资产经**匿名 curl（无凭据）** HTTP 200 / 9,356,437B / md5 与本地发布件逐字一致；v1.0.0~v1.0.3 未回改（五枚并列在册）。对外文案沿 S5-R12 边界：写"每轮之间等待你设定的秒数，带 ±1 秒随机抖动""载入不会跳过任何一道安全闸"，"可绕过平台检测"一句任何对外件皆零命中。**待裁两件已照登不擅动**：手动 `wait ms` 无上限（另立上限＝同一本账第二份口径）／`EMPTY_LEDGER` 载入空账档位话术。**遗留四件不变**：keystore 正式签名版（签名版需先卸载重装，上架前回炉）、Gumroad 上架动作归老板面、买家件未经母语校对、`byok-smoke` 7 行欠格与 K40 欠格并入下一次真机批。另勘正在册：此前账上引用的 commit `a5f1a63` **对象不存在**（`git cat-file -t` → Not a valid object name），非"未推"，已在 RULINGS/METRICS 明写并给出真实未推集。 | 同左 | 工单 `orders/ANYTOUCH-S5-delivery-ORDER.md`（§4 交付账 S5-c 行 + v1.0.2 行 + v1.0.3 行 + v1.0.4 行已填）、裁决 `orders/RULINGS-20260922.md` S5 批表（含 R5 顺序令原文、R10 三裁与执行落地段）；两份评审原件在桌面立项大会目录（队友文件只读，不入本仓） |
| W 轨 Windows 先行验证 | 🔒 老板拍板人力 | — | RULINGS P1-2 |

## 老板待办（阻塞项）

1. T2 配给：NVIDIA NIM 通道**已到位并结案首批**（15/15 Key 验真，端到端编译链打通）；仍缺 BYOK 另三档（Gemini/GPT-4o-mini/Haiku，配给仍在老板面）。~~需老板回禀：映射表 7 模型名全错，来源考据+重写待裁~~ → **勘误回禀结案（老板 S5-R4 令"直接按实测可用的 8 个模型改了"，已执行）**：`key.txt`（仓库外）表 7 行按本晚逐款 HTTP 实测重写（200 共 10 款、表用 7+备 1，404/503/超时不选），Key 行 15 把零改动、旧错名残留 0、备份 `key.txt.bak-20260924`；新设备事实入册：**目录在册≠可调用**（本晚实探 29 款、17 款在册却 404），对目录写表会再坏一次，认实测不认目录。明细 `evidence/S5/nim-model-map-rewrite.md`
