...
2. T3 配给：**小米真机已到位并完成首触结案（K40 国行 Android 13/MIUI V816）**——三大地基项过验+第 12 雷修复；**待老板裁三案**：①MIUI 搜索框 SET_TEXT/PASTE 双拒是否加第三通道（IME 代理属新增面，暂不）；②scroll 被 ROM 容器拒后是否允许 dispatchGesture 兜底（涉坐标红线，需明示）；③C5 高危链真机轮需英文界面（本机中文 MIUI 无 "password" 节点可命中，建议刷国际版或切系统语言）。三星/Pixel/摩托配给仍待
