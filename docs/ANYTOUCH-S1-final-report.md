...
1. **doze 冻结**：`moveTaskToBack` 后进程转 cached，系统冻结进程 → Main 线程协程轮询整体停摆，`rootInActiveWindow` 永远"未就绪"。修法：执行期挂前台服务（FOREGROUND_SERVICE_SPECIAL_USE + specialUse 型，34 三参 startForeground，ROM 拒绝则退两参）。
