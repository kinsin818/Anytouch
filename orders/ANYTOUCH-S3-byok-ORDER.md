...
| E-key（切片 E 剩下一半） | **未开始·阻塞在老板的手指上** | 需：①K40（或任一真机）在场、装本轮 debug APK、小米系两开关（雷 14）；②**在手机上用手**填 https 端点/模型名/Key 并点「保存」（**S3-R4-2：不走 adb 下发**；脚本一个 Key 字面量都没有、也从不读 Key 输入框）；③`ANDROID_SERIAL=<serial> bash scripts/byok-smoke.sh` → E5~E11 自动接管。**两项请示**：这组要打**两次**真请求（裁 4 字面"一次"，多出那次为量 §3-9 的秒级互斥窗口，不点头即跳过 E8）；`E_WIPE=1` 才跑「清除双槽」（会真删屏上凭据、删完需再手输，故默认不动）。**已认的边界**：模拟器 ≠ 真机，四条 JVM 覆盖 0 的设备点（真 HTTPS / 真 Keystore 硬件背书 / 词表真采几条 / 编译期双拒）本批**一条都没证**；`acceptModelActions` 内层 `model ledger refused gate=RUNNING` 与"AI 产物不被 V-3 误拒"（E5h）只有真产物能触发，仍在此账上 |
