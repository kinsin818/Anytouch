 ...
 *
 * **S5-g 之后的身份变了**（老板 S5-R14 裁 1 覆盖 S5-R13 裁 1）：本文件从"放行权威"降级为
 * **前置过滤**——格式判对只等于"有资格去问服务器"，真解锁以 `ActivationStore.submit` 拿到
 * 服务器 `ok:true` 为准（附页 §2）。降级没动这里任何一个判据：v1.0.5 那六档本地拒因逐字保留，
 * 它省掉的是"脏码白跑一次网络"，保住的是"五种手滑各说一句不同的话"。
 */
enum class ActivationVerdict {
    /**
     * 本地判据全对。**同一个档位名在两个入口意思不同，这一条是刻意保留的**：
     * - [ActivationCode.classify] 回 UNLOCKED＝"形状对，有资格去问服务器"（v1.0.6 起**不再等于解锁**）；
     * - [ActivationStore.submit] 回 UNLOCKED＝"形状对 **且** 服务器认了 **且** 本机落盘复核过了"。
     * 不改名的理由：这套档名字面与发卡口 `scripts/activation-code.py` 的输出同源（golden 表两语言对拍），
     * 换名等于把两份实现的公共词表拆成两套——那才是真雷。凡"这台机器算不算解锁"一律只认 [ActivationStore] 那一侧。
     */
    UNLOCKED,
    ...
    /**
     * 形状与校验位都对，但**服务器说这枚码不在册**（S5-g 军令 §2）。
     * 这一档存在本身就是本批的语义变化：v1.0.5 到此为止算解锁，v1.0.6 起不算。
     */
    SERVER_INVALID,

    /** 服务器说这枚码已经绑满两台，本机不在那两份里（军令 §1 的额度锁）。 */
    SERVER_SEATS_FULL,

    /** 连不上／超时／证书指纹对不上／答非所问——**fail-closed 的一档，绝不写成"码无效"**（自钉 2）。 */
    SERVER_UNREACHABLE,

    /** 本机给不出设备标识（`ANDROID_ID` 为空），请求根本没出门；与网络无关，不许混进上一档。 */
    DEVICE_ID_MISSING,
