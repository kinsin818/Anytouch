...
object ByokPreflight {
    ...
    fun copyOf(gate: Gate): String = when (gate) {
        Gate.READY -> "预检通过（这一档不该上屏：READY 的文案是地址知情回显，见 ByokPlan.notice）"
        Gate.EMPTY_INTENT -> "先说一句要做什么：意图框是空的，本次一个字都没发给模型。"
        Gate.RUNNING -> "有任务正在执行，AI 编译先不跑：编译产物是整本替换步序账的，执行中换账会让" +
            "正在跑的那一跑和屏上的账变成两套。请先点悬浮球停止，或等这一跑结束。"
        Gate.BUSY_COMPILE -> "上一次 AI 编译还没回来，不开第二跑（同一句话烧两次钱，而且后回来的那份会盖掉先回来的）。"
        Gate.NO_CREDENTIAL -> "本机还没有保存好的凭据（Key / 服务地址 / 模型名三样一起存）。请在下方填好并点「保存」，" +
            "屏上出现 Key 尾 4 位才算真存上了。"
        Gate.UNREADABLE -> "本机凭据现在读不出来，请在下方重新填一次 Key 并保存。"
        Gate.NO_MODEL -> "已保存的凭据里没有模型名，编译不知道要请求哪个模型。请在下方补上模型名并重新保存。"
        Gate.BAD_BASE_URL -> "已保存的服务地址现在过不了地址政策，本次没有在网络上发出任何一个字节。请重新保存一次地址。"
    }
    ...
    fun checkSave(key: String, baseUrl: String, model: String): String? = when {
        key.isBlank() -> "Key 是空的：空的 Key 存进本机只会让人以为已经配好，下次编译照样连不上。"
        model.isBlank() -> "模型名是空的：三样一起存才是完整配置（一个 blob 装三个字段，不留半条）。"
        else -> when (val policy = BaseUrlPolicy.check(baseUrl)) {
            is BaseUrlPolicy.Ok -> null
            is BaseUrlPolicy.Fail -> "服务地址不能保存：" + policy.message + "（本机一个字节都没写。）"
        }
    }
}
