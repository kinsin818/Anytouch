...
object AppState {
    ...
    /**
     * 输码那一格的结论（v1.0.5 的六种"这枚码不对"加一种"设备不肯写"，再加 S5-g 的四种
     * "形状对但服务器那格没给过"：在册否／额度满／连不上／本机没标识）。与 [activated] 同源不同事：
     * 前者是"这一次输码为什么没成"，后者是"这台机器现在解没解锁"。
    ...
    /**
     * 正在等服务器那一格（S5-g 起输码是异步的：本地判据当场出结论，服务器那一格最慢 10 秒）。
     * 这一格存在的全部理由：**没有它，点了 Unlock 之后那 8 秒就是"屏幕没反应"**——买家会再点一次、
     * 再点一次，而每一次点击都可能已经在服务器上占掉一格额度（静默=黑洞的上屏版）。
     * 它只让按钮置灰并显示那一句"checking"，不参与任何判据。
     */
    val activationBusy = MutableStateFlow(false)
